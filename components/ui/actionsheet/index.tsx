...
// Bottom padding is applied at runtime via the safe-area inset (see
// ActionsheetContent) so it survives sheets that override padding with `p-0`
// (which would otherwise wipe a `pb-safe` here, leaving action buttons flush
// against the home indicator). This keeps every sheet's bottom breathing room
// consistent with the FormLayout footer (safe-area inset + the footer's own p-4).
...
>(function ActionsheetContent({ className, style, ...props }, ref) {
  ...
  return (
    <UIActionsheet.Content
      ...
      // Inline style is merged after the className-derived styles, so this
      // bottom inset wins even when `className` is `p-0`.
